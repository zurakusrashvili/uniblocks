    ...
    <div class="lg:grid lg:grid-cols-3">
      <div class="h-screen relative col-span-3 h-screen overflow-y-auto">
        <!-- <div class="flex flex-row gap-4 items-center fixed lg:absolute top-4 z-20 right-4">
          ...
          <button aria-label="show JSON" class="lg:hidden w-12 h-12 bg-slate-800 text-white rounded-full border-2"
            @click.prevent="showContent = !showContent">
            { }
          </button>
        </div> -->

        <uniblocks editorClass="my-32 prose xl:prose-xl text-slate-600 max-w-none" v-model="content" :editable="editable"
          mode="json" :blockTools="blockTools" :extensions="extensions" :blockWidthTypes="[
          'vueComponent',
          'horizontalRule',
          'blockquote',
          'youtube',
